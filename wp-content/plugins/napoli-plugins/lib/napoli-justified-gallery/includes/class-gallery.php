...
class DGWT_JG_Gallery {
	...
	public function post_gallery( $output, $attr, $instance = 0) {
		...
		$output = '<div id="' . $selector . '" data-height="' . $this->options['row_height'] . '" class="dgwt-jg-gallery dgwt-jg-style1 ' . $selector . '">';
		...
		foreach ( $attachments as $img_id => $attachment ) {
			...
			$html_img = '<img src="'. (is_array($image_src_t) ? $image_src_t[0] : $image_src_t) .'" alt="">';
