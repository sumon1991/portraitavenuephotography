...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      print '<a href="#" class="button button-primary cs-shortcode" data-editor-id="'. $editor_id .'">'. __( 'Add Shortcode', CS_TEXTDOMAIN ) .'</a>';
    }
  ...
  ?>
    <div id="cs-shortcode-dialog" class="cs-dialog" title="<?php _e( 'Add Shortcode', CS_TEXTDOMAIN ); ?>">
      <div class="cs-dialog-header">
        <select class="chosen <?php print ( is_rtl() ) ? 'chosen-rtl' : ''; ?> cs-dialog-select" data-placeholder="<?php _e( 'Select a shortcode', CS_TEXTDOMAIN ); ?>">
          <option value=""></option>
          <?php
            foreach ( $this->options as $group ) {
              print '<optgroup label="'. $group['title'] .'">';
              foreach ( $group['shortcodes'] as $shortcode ) {
                $view = ( isset( $shortcode['view'] ) ) ? $shortcode['view'] : 'normal';
                print '<option value="'. $shortcode['name'] .'" data-view="'. $view .'">'. $shortcode['title'] .'</option>';
              }
              print '</optgroup>';
            }
          ?>
  ...
  public function shortcode_generator() {
    ...
    if( isset( $shortcode['fields'] ) ) {
      ...
      foreach ( $shortcode['fields'] as $key => $field ) {
        ...
        print cs_add_element( $field, $field_default, 'shortcode' );
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      print '<div class="cs-shortcode-clone" data-clone-id="'. $clone_id .'">';
      print '<a href="#" class="cs-remove-clone"><i class="fa fa-trash"></i></a>';
      ...
      print '<div class="cs-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
